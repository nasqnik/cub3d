...
static int check_collision_x(t_info *info, t_move *move)
{
    ...
    // if ((wall_dist_x > COL_BUFFER && info->map[move->current.y][wall_cell_x] == '1')
	// 	|| info->map[move->current.y][wall_cell_x] != '1')
	// 	return (0);
    if ((wall_dist_x > COL_BUFFER && info->example_map[move->current.y][wall_cell_x] == 1)
		|| info->example_map[move->current.y][wall_cell_x] != 1)
		return (0);
	return (1);
}
...
static int check_collision_y(t_info *info, t_move *move)
{
    ...
    // if ((wall_dist_y > COL_BUFFER && info->map[wall_cell_y][move->current.x] == '1')
	// 	|| info->map[wall_cell_y][move->current.x] != '1')
	// 	return (0);
    if ((wall_dist_y > COL_BUFFER && info->example_map[wall_cell_y][move->current.x] == 1)
		|| info->example_map[wall_cell_y][move->current.x] != 1)
		return (0);
	return (1);
}
...
int check_map_boundaries(t_info *info, t_move *move)
{
	...
	if (move->new.x < 0 || move->new.x >= move->map_new_y_len || move->current.x < 0
		...
		return (1);
	// if (info->map[move->new.y][move->new.x] == ' '      // check with our map
	// 	|| info->map[move->current.y][move->new.x] == ' '
	// 	|| info->map[move->new.y][move->current.x] == ' ')
	// 	return (1);
	// wall_ahead = (info->map[move->new.y][move->new.x] == '1');   // check with our map
	// wall_x = (info->map[move->current.y][move->new.x] == '1');
	// wall_y = (info->map[move->new.y][move->current.x] == '1');
    wall_ahead = (info->example_map[move->new.y][move->new.x] == 1);
    wall_x = (info->example_map[move->current.y][move->new.x] == 1);
    wall_y = (info->example_map[move->new.y][move->current.x] == 1);
...
int check_map_position(t_info *info, t_move *move)
{
    ...
    move->map_new_y_len = info->map_width;
    ...
    if (info->example_map[move->current.y][move->new.x] != 1 && !check_collision_x(info, move))
		info->player.pos.x += move->step.x;
    if (info->example_map[move->new.y][move->current.x] != 1 && !check_collision_y(info, move))
		info->player.pos.y += move->step.y;
